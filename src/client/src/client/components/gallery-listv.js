    ...
    return(
        <React.Fragment>
            {props.gallerylisterv.map((item) => (
                <div className="col-md-4 filter photo" key={item.id} style={{marginBottom: '2rem'}}>
                    <div className="each-item gallery_img" onClick={props.view_photov.bind(this, item.id)}>
                        <video className="port-image img-fuild" src={`assets/images/gallery/${item.name}`} alt="" muted/>
                    </div>
